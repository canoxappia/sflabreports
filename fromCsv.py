...
tableUnpivot.to_csv(os.path.join(os.path.dirname(__file__), 'CategoriaContenido.csv'), header=True)


############################
#Filtro: Se crea un nuevo csv con filtro en los valores verdaderos de la columna Value creada luego del unpivot
with open(os.path.join(os.path.dirname(__file__), 'CategoriaContenido.csv'), 'r') as inputCSV, open(os.path.join(os.path.dirname(__file__), 'CategoriaContenidoUnpivot.csv'), 'w') as outputCSV:
    outputCSV = csv.writer(outputCSV)
    inputCSV = csv.reader(inputCSV)
    for row in inputCSV:
        if row[7] == 'verdadero' or row[7] != 'falso' :
